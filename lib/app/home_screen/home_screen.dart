                        ...
                        Expanded(
                          child: Constant.mapType == "inappmap"
                              ? Constant.selectedMapType == "osm"
                                  ? Obx(() {
  // This will trigger when currentOrder changes
  if (controller.currentOrder.value.id != null) {
    // Trigger route calculation when order changes
    WidgetsBinding.instance.addPostFrameCallback((_) {
      controller.initializeMapForCurrentOrder();
    });
  }
  ...
  return flutterMap.FlutterMap(
    ...
    options: flutterMap.MapOptions(
      initialCenter: location.LatLng(
        controller.driverModel.value.location?.latitude ?? 0.0,
    ...
    children: [
      ...
      flutterMap.MarkerLayer(
        markers: controller.currentOrder.value.id == null 
          ? [] 
            ...
            flutterMap.Polyline(
              ...
              color: AppThemeData.primary300,
                        ...
                        controller.currentOrder.value.id != null &&
                                controller.currentOrder.value.status !=
                                    Constant.driverPending &&
                controller.currentOrder.value.status != "Order Completed"
    ...
    return Container(
      ...
      child: Column(
        children: [
          ...
          InkWell(
            onTap: () async {
              if (controller.currentOrder.value.status == "Order Completed") {
                 controller.update();
    return; 
  }
...
if (value == true) {
  try {
    ...
    final orderId = controller.currentOrder.value.id;
    
    if (orderId != null) {
      controller.driverModel.value.inProgressOrderID!.remove(orderId);
      ...
      FireStoreUtils.getUserProfile(controller.driverModel.value.id!).then((updatedDriver) {
        if (updatedDriver != null) {
          controller.driverModel.value = updatedDriver;
          controller.update();
        }
      });
    }
    ...
    ShowToastDialog.showToast("Failed to complete delivery. Please try again.");
  }
}
                  },
                );
